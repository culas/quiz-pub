...
<div>
	...
	{:else if $qService.value.round}
		...
		{#each $qService.context.questions.filter((q) => q.roundId === cr) as q}
			...
			{#each $qService.context.answers.filter((a) => a.roundId === cr && a.questionId === q.id) as a}
				<section>
					<div>
						...
						{#if $qService.value.round === 'revealing'}
							<button on:click={() => qService.send('REVEAL', { qIdx: q.id, player: a.player })}
								>reveal</button
							>
						{:else if $qService.value.round === 'scoring'}
							<button
			...
			{/each}
			{#if $qService.value.round === 'answering'}
